...
// بيدوّر على مكان ملف الواجهة (index.html) تلقائياً، سواء كانت المشروع منظم
// في فولدرين منفصلين (backend/ و frontend/) أو كل الملفات في نفس المكان
// (زي ما هو الحال في الريبو الحالي على GitHub، من غير فولدرات فرعية). كده
// السيرفر بيشتغل صح مهما كان ترتيب الملفات، من غير ما تحتاج تعيد تنظيم أي حاجة.
const fs = require('fs');
function resolveFrontendDir() {
  if (process.env.KODAK_FRONTEND_DIR) return process.env.KODAK_FRONTEND_DIR;
  const candidates = [
    path.join(__dirname, '..', 'frontend'), // ترتيب فولدرين منفصلين: backend/ + frontend/
    __dirname,                              // كل الملفات في نفس الفولدر (index.html جنب server.js)
  ];
  for (const dir of candidates) {
    if (fs.existsSync(path.join(dir, 'index.html'))) return dir;
  }
  // معملناش لقاه في أي مكان متوقع - نرجع أول احتمال بردو (هيظهر خطأ واضح
  // "Cannot GET /" بدل ما يفشل السيرفر بالكامل، وده أوضح للتشخيص).
  return candidates[0];
}
const FRONTEND_DIR = resolveFrontendDir();
console.log(`ℹ️  بيقدّم ملفات الواجهة من: ${FRONTEND_DIR}`);
...
const pool = new Pool({
  ...
  // "نايم" لعدم الاستخدام)، أي طلب بيحتاج قاعدة بيانات كان هيفضل "معلّق" (Pending)
  // على المتصفح للأبد من غير أي رسالة خطأ - وده اللي بيحس المستخدم إنه "مشلول"
  // من غير ما يعرف السبب. دلوقتي أي محاولة اتصال بتفشل بوضوح خلال 8 ثواني بدل
  // ما تستنى للأبد، والمستخدم بيشوف رسالة خطأ واضحة على طول.
